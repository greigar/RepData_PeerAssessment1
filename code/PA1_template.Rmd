...
library(tidyr)     # for tidying and shaping data
library(knitr)     # for using kable to generate tables
```

Load the zipped CSV data (using readr's read_csv) into a data set (tibble) called activity.
...
Therefore, no further processing is required since readr's read_csv() correctly detects the variable data types.
...
ggplot(activity_by_interval, aes(x = interval, y = step_mean) ) +
    geom_line() +
    labs(x = "Measurement Interval (5 minute)", y = "Average Number of Steps") + # set label text
...
```{r}
activity_na  <- activity %>% filter(is.na(steps)) %>% group_by(date) %>% summarise(na_count = n())
kable(activity_na, caption = "Days with missing step data", col.names = c("Date", "Number of missing intervals"))
```
...
So, perhaps we can use the average of *equivalent* day intervals - so data missing for Monday is assigned the average of all the other Mondays.
...
Create a data set `activity_weekday_mean` to hold the average steps per interval for each weekday.
...
This function will take a date for a day which is missing step data and return a data set containing the average steps per for each interval for the date's week day.
```{r}
...
Values are effected for total number of steps.
...
By imputing missing values, we have added `r sprintf("%7.2f", steps_added)`.
...
This function translates a week day date into a label indicating whether it is a week day or weekend.
```{r}
